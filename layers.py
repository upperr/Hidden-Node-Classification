...
class GraphConvolution(Layer):
    """Degree-gated GNN
    """
    def __init__(self, input_dim, output_dim, gate_dim, act = tf.nn.relu, dropout = 0., **kwargs):
        ...
        with tf.compat.v1.variable_scope(self.name + '_degree_vars'):
            self.vars['weight_degree1'] = weight_variable_glorot(1, gate_dim, name = "weight_degree1")
            self.vars['bias_degree1'] = tf.Variable(tf.zeros((1)), name = "bias_degree1")
            self.vars['weight_degree2'] = weight_variable_glorot(gate_dim, 1, name = "weight_degree2")
...
class Dense(Layer):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, dropout = 0., act = tf.nn.relu, bias = True, **kwargs):
        ...
        with tf.compat.v1.variable_scope(self.name + '_vars'):
            #self.vars['weights1'] = weight_variable_glorot(input_dim, FLAGS.decoder, name = "weights1")
            ...
            #self.vars['weights2'] = weight_variable_glorot(FLAGS.decoder, output_dim, name = "weights2")
